...
class ApiConsoleExtComm extends PolymerElement {
  ...
  disconnectedCallback() {
    ...
    if (this._oldEventTarget) {
      this._unobserverConsoleEvents(this._oldEventTarget);
      this._oldEventTarget = undefined;
    }
  }
  /**
   ...
   */
  _eventTargetChanged(et) {
    if (this._oldEventTarget) {
      this._unobserverConsoleEvents(this._oldEventTarget);
    }
    this._oldEventTarget = et;
  ...
  _responseReady(data) {
    ...
    const logs = data.log;
    if (logs && logs.length) {
      logs.forEach((item) => console.info(`[API console proxy] ${item}`));
    }
